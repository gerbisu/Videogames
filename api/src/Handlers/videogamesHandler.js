...
const postVideogameHandler =  async (req,res)=>{
    const {name,description,platforms,image,released,rating,genre}= req.body; // todos los datos del MODEL de videgoames para guardarlo en la DB
    try{
        if(!name||!description||!platforms||!image||!released||!rating||!genre) throw new Error("Faltan Datos");
        const newVideoGame = await createVideogameDB(name,description,platforms,image,released,rating,genre);
